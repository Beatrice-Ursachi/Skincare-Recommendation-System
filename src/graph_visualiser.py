...
def plot_top_good_ingredients(result):
    ...
    plt.savefig(folder / 'plot_top_good_ingredients.png')
    plt.close()
...
def plot_top_bad_ingredients(result):
    ...
    plt.ylabel('Frequency', fontsize=12)
    plt.xticks(rotation=45)
    plt.savefig(folder / 'plot_top_bad_ingredients.png')
    plt.close()
...
def plot_score_distribution(product_df1):
    ...
    plt.savefig(folder / 'plot_score_distribution.png')
    plt.close()
...
def plot_score_vs_counts(product_df1):
    ...
    plt.savefig(folder / 'plot_score_vs_counts.png')
    plt.close()